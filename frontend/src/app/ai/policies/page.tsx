...
import {
  SALES_POLICIES,
  evaluateLeadAgainstPolicies,
  type LeadOrchestratorOutput,
  type EvaluationLogEntry,
} from '@/components/ai/insights/salesPolicies'

// ============================================================================
// Persistence — best-effort local persistence of user edits.
// ASSUMPTION: no backend endpoint was provided, so edits are persisted to
// localStorage keyed by policy id, layered on top of the hardcoded base
// definitions above. Swap this for a real API call when one exists.
// ============================================================================

const STORAGE_KEY = 'sales-policies:overrides:v1'

type PolicyOverride = Partial<
  Pick<Policy, 'is_active' | 'priority' | 'dsl' | 'name' | 'description' | 'tags'>
>
...
function saveOverrides(overrides: Record<string, PolicyOverride>) {
  if (typeof window === 'undefined') return
...
export default function AIPoliciesPage() {
  ...
  // Evaluation logging (in-memory, most recent first)
  const [evaluationLogs, setEvaluationLogs] = useState<EvaluationLogEntry[]>([])
  ...
  const [structuredDSL, setStructuredDSL] = useState<{
    conditions: Array<{
      field: string
      operator: string
      value: string
  ...
  // ==========================================================================
  // Data — hardcoded base sales policies, layered with persisted user overrides.
  // Replace with a real API fetch when a backend endpoint exists.
  // ==========================================================================
  ...
  const loadPolicies = useCallback(() => {
    ...
    setPolicies(applyOverrides(SALES_POLICIES, overrides))
  ...
  const persistOverride = useCallback(
    (id: string, patch: PolicyOverride) => {
      const overrides = loadOverrides()
  ...
  const handleSavePolicy = useCallback(async () => {
    // ASSUMPTION: PolicyEditModal's onSave doesn't return the updated policy
    // here, so we reload from the base + override store. If PolicyEditModal
    // is wired to persist to a backend, point persistOverride at that call
    // instead of localStorage.
    loadPolicies()
  }, [loadPolicies])

  const togglePolicyStatus = useCallback(
    async (id: string, _isActive: boolean) => {
      setPolicies((prev) => {
  ...
  const deletePolicy = useCallback(async (id: string) => {
    // Deleting one of the four base sales gates removes it from view/evaluation
    // for this browser only (persisted as an override), since these are the
    // required baseline policies rather than user-created ones.
  ...
  // Create-with-AI / Structured Builder still work, but new policies are
  // additional and layered in on top of the four required sales gates.
  ...
  const runEvaluation = useCallback(
    (leadOutput: LeadOrchestratorOutput) => {
      const result = evaluateLeadAgainstPolicies(
        policies,
        leadOutput
      )
  ...
  const filteredPolicies = policies
    .filter((policy) => {
      ...
      if (filter === 'logical' && policy.policy_type !== 'logical') {
        return false
      }
      if (
        filter === 'natural_language' &&
        policy.policy_type !== 'natural_language'
      ) {
        return false
      }
  ...
  const stats = {
    ...
    structured: policies.filter(
      (p) => p.policy_type === 'logical'
    ).length,
    natural: policies.filter(
      (p) => p.policy_type === 'natural_language'
    ).length,
  }
                ...
                <div className="max-w-3xl mx-auto">
                  ...
                  <div className="mb-6">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setStructuredName(e.target.value)
                      }
                  ...
                  <div className="flex justify-center gap-3 mt-8">
                    ...
                    <Button
                      ...
                      disabled={
                        !structuredDSL ||
                        structuredDSL.conditions.length === 0 ||
                        !structuredName.trim() ||
                        isSavingStructured
                      }
                      onClick={async () => {
